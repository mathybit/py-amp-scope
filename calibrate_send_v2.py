...
def _play_one_freq_single(
    ...
    try:
        ...
        elapsed = 0
        total_s = switcher.total_out_samples / fs
        while in_offset[0] < total_out_samples * fs / fs and elapsed < total_s * 3:
            sd.sleep(100)
            elapsed += 0.1
...
def main():
    ...
    if args.dry_run:
        ...
        print(f"\n[Dry run profile saved to {output_dir / 'v2_cal_send_profile.npz'}]")
        return
    ...
    if args.mode == "sequential":
        ...
        for i, target_freq in enumerate(freq_array):
            print(f"\n  [{i + 1}/{num_freqs}] {target_freq:.1f}Hz", flush=True)
            try:
            ...
            print(f"  RMS={rms:.6f}  {target_freq:.1f}Hz@{db_target:.1f}dBFS", flush=True)
    ...
    for freq, amp, rms in results:
        if np.isnan(amp):
            status = "SKIP"
        elif amp > -60 and amp < 75 and rms < 1.0:
            status = "GOOD"
        elif amp <= -80 or rms >= 1.0:
            status = "WEAK"
        else:
            status = "CLIP"
    ...
    if valid_results:
        ...
        # Build correction filter and chart (same as v1)
        png_bytes = build_chart_png(
            freqs=valid_freqs, H=amp_array[:len(valid_amps)], title="Send Calibration Response (v2)",
        )
        metadata = {
            "method": method,
            "mode": args.mode,
        ...
        # Save response in linear magnitude (not dB) for compatibility with inverse filter
        if valid_results:
            amp_linear = 10 ** (valid_amps / 20.0)  # convert dB to magnitude
        else:
        ...
        npz_path = save_cal_profile(
            ...
            freqs=valid_freqs if valid_results else freq_array,
        )
        ...
        if args.correct:
            # Approximate inverse from magnitude response (phase=0 for simplicity)
